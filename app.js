...
var showAnswerer = function(item) {
	...
	// display top answerer name with link to profile
	var displayElem = result.find('.answerer');
	displayElem.html('<a target="_blank" href=http://stackoverflow.com/users/' + item.user.user_id + '>' + item.user.display_name + '</a>');	
	
	// display top answerers post count for the subject
	var postElem = result.find('.post-count');
	postElem.text(item.post_count);
